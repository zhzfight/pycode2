...
def cur_preference_vec_gen():
    ...
    cur_preference_vec = pd.DataFrame(
        columns=['userID', 'VenueId', 'Time_Slot', 'Time', 'Region'] + [x for x in range(100)])
    ...
    for index, row in cur_preference_vec.iterrows():
        ...
        pois_time = train_data[(train_data[1] == row['userID'])& (row['Time']-train_data['Time'] < delta_t)&(row['Time']-train_data['Time'] > 0) ][[3,'Time']]
        ...
        for index_, row_ in pois_time.iterrows():
            ...
            vec = vec +  p_v
        cur_preference_vec.loc[index,[x for x in range(100)]] = vec
        bar.next()
    bar.finish()
...
def topNscore():
    ...
    for index, row in cur_preference_vec.iterrows():
        ...
        for key, value in poi_vec_dict.items():
            ...
            # score = ( cp_vec + region_vec +  time_slot_vec+user_vec).dot(value)
